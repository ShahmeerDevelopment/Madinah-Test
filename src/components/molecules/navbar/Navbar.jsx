...
const AuthButton = ({ isLogin, handleOpenModal, transparent }) => {
  ...
  const [auth] = useState(getCookie("token")); // Use getCookie to retrieve the token
  // Check if src query param is not internal_website
...
export default function Navbar({ isWidgetMode = false }) {
  ...
  const [auth, setAuth] = useState(reduxToken);

  useEffect(() => {
    setAuth(reduxToken || getCookie("token"));
  }, [reduxToken]);
