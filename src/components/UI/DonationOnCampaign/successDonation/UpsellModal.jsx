...
const UpsellModal = ({ upSells, onNoButtonClick, onYesButtonClick }) => {
  ...
  const updatePaymentPayloadForUpsell = (paymentPayload, upsellAmount) => {
    ...
    updatedPayload.donationAmount = upSells[currentIndex]?.isRecurring
      ? 0
      : upsellAmount;

    const tipAmount = 0;
    updatedPayload.tipAmount = 0;
    updatedPayload.recurringAmount = upSells[currentIndex]?.isRecurring
      ? upsellAmount
      : 0;
    updatedPayload.totalAmount = (upsellAmount + tipAmount).toFixed(2);
    updatedPayload.sellId = upSells[currentIndex]._id;
    updatedPayload.isRecurring = upSells[currentIndex].isRecurring
      ? true
      : false;
    updatedPayload.recurringType = upSells[currentIndex].recurringType;
  ...
  const handleYesButtonClick = async () => {
    ...
    const updatedPaymentPayload = updatePaymentPayloadForUpsell(
      paymentPayload,
      upSells[currentIndex].amount,
    );
    const successAmountPayload = {
      ...updatedPaymentPayload,
      donationAmount: upSells[currentIndex].amount,
      recurringDonation: upSells[currentIndex]?.isRecurring
        ? upSells[currentIndex]?.amount
        : 0,
      symbol: paymentPayload?.symbol,
      title: upSells[currentIndex]?.title,
    };
    ...
    const successPayload = {
      ...
      isRecurringPaymentSelected: upSells[currentIndex].isRecurring
        ? true
        : false,
    };
        ...
        postCardPayment(
          ...
          .then((res) => {
            ...
            if (res?.data.success) {
              ...
              // Handle upsell progression only on successful payment
              if (currentIndex < upSells.length - 1) {
                // If there's another upsell item, show it
                setCurrentIndex(currentIndex + 1);
              } else {
                // If currently showing the last upsell item, close the modal
  ...
  const getRecurringTypeTitle = () => {
    const recurringTypeItem = AUTOMATIC_DONATION_DAYS.find(
      (item) => item.value === upSells[currentIndex]?.recurringType,
    );
  ...
  return (
    <StackComponent direction={"column"} alignItems="center">
      ...
      >
        {upSells[currentIndex]?.subTitle}
      </SubHeading>

      <CoverImagePreview imageSrc={upSells[currentIndex]?.imageUrl} />
      ...
      >
        <TypographyComp
          sx={{
            ...
            lineHeight: "22px",
            color: "#424243",
            wordWrap: upSells[currentIndex]?.description
              ?.split(" ")
        ...
        >
          {upSells[currentIndex]?.description}
        </TypographyComp>
      </BoxComponent>
      <StackComponent
        ...
        sx={{ width: "100%" }}
      >
        {upSells[currentIndex]?.recurringType ? (
          <BoxComponent
            sx={{
              borderRadius: "22px",
          ...
          >
            ...
            >
              ...
              >
                {upSells[currentIndex]?.yesButtonText}
              </LoadingBtn>
            </BoxComponent>
          </BoxComponent>
        ) : (
        ...
        >
          {upSells[currentIndex]?.noButtonText}
        </ButtonComp>
      </StackComponent>
    </StackComponent>
  );
...
UpsellModal.propTypes = {
  ...
  id: PropTypes.any,
  upSells: PropTypes.any,
  onNoButtonClick: PropTypes.any,
  onYesButtonClick: PropTypes.any,
};
